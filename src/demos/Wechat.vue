...
<div>
  ...
  <group>
    <switch title="通知显示消息详情" :value.sync=true></switch>
  </group>
  <tip>关闭后，当收到微信消息时，通知提示将不再显示发信人和内容摘要。</tip>

  <group>
    <cell title="功能消息免打扰" link="test"></cell>
  </group>
  <tip>设置系统功能消息提示声音和振动的时段。</tip>

  <group>
    <switch title="声音" :value.sync=false></switch>
    <switch title="振动" :value.sync=true></switch>
  </group>
  <tip>当微信在运行时，你可以设置是否需要声音或者振动。</tip>

  <group>
    <switch title="朋友圈照片更新" :value.sync=true></switch>
  </group>
